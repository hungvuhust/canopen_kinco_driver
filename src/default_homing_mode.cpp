...
bool DefaultHomingMode::read(const uint16_t &sw) {
  ...
  status_= (sw & (MASK_Reached | MASK_Attained | MASK_Error | MASK_QuickStop)) ^
           (MASK_QuickStop);
...
bool DefaultHomingMode::executeHoming() {
  ...
  if (!cond_.wait_until(
          lock, prepare_time,
          masked_status_not_equal<
              MASK_Error | MASK_Attained | MASK_Reached, MASK_Reached>(
              status_))) {
  ...
  if (!cond_.wait_until(
          lock, finish_time,
          masked_status_not_equal<
              MASK_Error | MASK_Attained | MASK_QuickStop, 0>(status_))) {
  ...
  if (!cond_.wait_until(
          lock, finish_time,
          masked_status_not_equal<
              MASK_Error | MASK_Reached | MASK_QuickStop, 0>(status_))) {
